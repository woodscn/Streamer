...
!!$    left = tempL ; right = tempR
    ...
    if( Ustar .gt. x )then
       ...
       if( PsiL .gt. 1.d0 )then
          ...
          elseif( cLH .lt. x )then
             ...
             out(3) = left(3) - 2.d0*aL/(gamma_const-1.d0)*((out(1)/left(1))**((gamma_const-1.d0)/(2.d0*gamma_const))-1.d0)
       ...
       if( PsiR .gt. 1. )then
          ...
          elseif( cRH .gt. x )then
             ...
             out(3) = right(3) + 2.d0*aR/(gamma_const-1.d0)*((out(1)/right(1))**((gamma_const-1.d0)/(2.d0*gamma_const))-1.d0)
  ...
  subroutine constoprim(main,nx,ny,nz)
    ...
    temp1 = 1.d0/main(21,:,:,:)
    temp2 = 1.d0/main(1,:,:,:)
    p = gamma2*( temp1*main(5,:,:,:) - .5d0*temp2*main(21,:,:,:)&
...
! Compute an orthonormal coordinate system given an initial, unnormalized vector.
  ...
  subroutine prim_update(main,dt,nx,ny,nz)
...
! an n + 1 index containing the contents prescribed by the boundary conditions.
    implicit none
    real(8), dimension(21,nx+2,ny+2,nz+2), intent(inout) :: main
!f2py intent(in,out) :: main
    ...
    do k = 1, nz + 1
       do j = 1, ny + 1
          do i = 1, nx + 1
             fluxes( 1: 5,1,i,j,k) = flux(riemann_solve(main(:,i,j,k),main(:,i+1,j,k)),&
                  .5d0*(main(:,i,j,k)+main(:,i+1,j,k)),1)
             fluxes( 1: 5,2,i,j,k) = flux(riemann_solve(main(:,i,j,k),main(:,i,j+1,k)),&
                  .5d0*(main(:,i,j,k)+main(:,i,j+1,k)),2)
             fluxes( 1: 5,3,i,j,k) = flux(riemann_solve(main(:,i,j,k),main(:,i,j,k+1)),&
                  .5d0*(main(:,i,j,k)+main(:,i,j,k+1)),3)
             fluxes( 6: 8,1,i,j,k) = 0.5d0*(main(15:17,i+1,j,k)+main(15:17,i,j,k))
             fluxes( 9:11,2,i,j,k) = 0.5d0*(main(15:17,i,j+1,k)+main(15:17,i,j,k))
             fluxes(12:14,3,i,j,k) = 0.5d0*(main(15:17,i,j,k+1)+main(15:17,i,j,k))
          end do
       end do
    end do
    write(*,*) "Fluxes = "
    write(*,*) fluxes(1,1,:,2,2)
    call primtocons(main,nx,ny,nz)
    main(1:14,2:nx+1,2:ny+1,2:nz+1) = main(1:14,2:nx+1,2:ny+1,2:nz+1) - (&
         (fluxes(:,1,2:nx+1,:,:)-fluxes(:,1,1:nx,:,:))*dxi*dzeta + &
