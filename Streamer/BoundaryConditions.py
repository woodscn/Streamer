...
class Face(object):
    ...
    def __call__(self,main_data,t,out,opts):
        ...
        for patch in self.patches:
            ...
            out = patch(main_data,t,opts,out)
    ...
    def _compute_computational_coordinates(self,patch,main_data,opts):
        ...
        for bounding_point in patch.bounding_points:
            # When you add new columns, it's possible that findclosestpoint
            # will find a point that is not in the first column of real data.
            # I'm not sure whether or not this is actually a problem.
...
class Patch(object):
    ...
    def __call__(self,main_data,t,opts,out):
        '''
        ...
        # does not contain ghost points. Therefore, main_data[:,i,j,k] (python
        #indexing) has xi-coordinates (i+xi_offset, j, k). So, I'll just grab
        # any point that falls within my bounding box. 
        min_xi   = np.floor(min(self.bounding_points_xi[:,0]))
        max_xi   =  np.ceil(max(self.bounding_points_xi[:,0]))
        min_eta  = np.floor(min(self.bounding_points_xi[:,1]))
        max_eta  =  np.ceil(max(self.bounding_points_xi[:,1]))
        min_zeta = np.floor(min(self.bounding_points_xi[:,2]))
