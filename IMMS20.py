...
Euler_UCS = Euler_UCS.Euler_UCS(
    Euler_UCS.MASA_solution_full(
        ranges=[[xmin, xmax], [ymin, ymax],
                [zmin, zmax]], nxes=(nx, ny, nz), dxis=dxis, disc=True))
...
def init():
    ...
    for inda in range(ny):
        for indb in range(nz):
            x_min_bcs[:-1, inda, indb] = Euler_UCS.sol.subs(
                dict(zip(Euler_UCS.vars_, (0., xmin, dxis[1] * float(inda),
                                           dxis[2] * float(indb))))).evalf()[:]
            x_max_bcs[:-1, inda, indb] = Euler_UCS.sol.subs(
                dict(zip(Euler_UCS.vars_, (0., xmax, dxis[1] * float(inda),
                                           dxis[2] * float(indb))))).evalf()[:]
    y_min_bcs = numpy.zeros((21, nx, nz))
    y_max_bcs = numpy.zeros((21, nx, nz))
    for inda in range(nx):
        for indb in range(nz):
            y_min_bcs[:-1, inda, indb] = Euler_UCS.sol.subs(
                dict(zip(Euler_UCS.vars_, (0., dxis[0] * float(inda), ymin,
                                           dxis[2] * float(indb))))).evalf()[:]
            y_max_bcs[:-1, inda, indb] = Euler_UCS.sol.subs(
                dict(zip(Euler_UCS.vars_, (0., dxis[0] * float(inda), ymax,
                                           dxis[1] * float(indb))))).evalf()[:]
    z_min_bcs = numpy.zeros((21, nx, ny))
    z_max_bcs = numpy.zeros((21, nx, ny))
    for inda in range(nx):
        for indb in range(ny):
            z_min_bcs[:-1, inda, indb] = Euler_UCS.sol.subs(
                dict(zip(Euler_UCS.vars_, (0., dxis[0] * float(inda), dxis[1] *
                                           float(indb), zmin)))).evalf()[:]
            z_max_bcs[:-1, inda, indb] = Euler_UCS.sol.subs(
                dict(zip(Euler_UCS.vars_, (0., dxis[0] * float(inda), dxis[1] *
                                           float(indb), zmax)))).evalf()[:]
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                xes = (0., dxis[0] * float(i), dxis[1] *
                       float(j), dxis[2] * float(k))
                initial_conds[:-1, i, j, k] = Euler_UCS.sol.subs(
                    dict(zip(Euler_UCS.vars_, xes))).evalf()[:]
                source_funcs[i, j, k] = lambda y, t, i=(dxis[0] * i), j=(dxis[1] * j), k=(dxis[2] * k) : numpy.array([item[0, 0] for item in manufactured_source_function(t, i, j, k)] + [0], dtype=numpy.float64)
    initial_conds[-1, :, :, :] = (initial_conds[5, :, :, :] * initial_conds[9, :, :, :] * initial_conds[13, :, :, :])
    exact_solution = lambda t, x, y, z : (numpy.array(Euler_UCS.sol.subs(dict(zip(Euler_UCS.vars_, (t, dxis[0] * x, dxis[1] * y, dxis[2] * z)))).evalf()[:]))
