...
where $A$,$B$,$C$, $a$, and $\mu$ are arbitrary constants, and $n$ is a positive integer.
 
The spatial coordinate $\xi$ is taken as
\begin{equation}
\label{eq:coord_rotation}
\xi = \cos\left(\phi\right)\cos\left(\theta\right)x-\cos\left(\phi\right)\sin\left(\theta\right)y+
\sin\left(\theta\right)z
\end{equation}
subject to the constraint $\phi = \theta$.

The value of $S'$ can be computed for varying values of $\phi$ in order to measure the accuracy of a particular IMMS implementation, and results for {\tt BACL-IMMS} are shown in Fig. \ref{fig:heat_exact_residual}
